...
> 
> Если внутри `MathExpression` требуется получить сырые декартовы координаты точек, точка передается в блок `variables` как обычная строка-ссылка. 
> 
> Внутри строки `expression` для извлечения координат используются строго определенные функции:
> * **`x(P)`** — извлекает абсциссу (координату $X$) точки $P$.
> * **`y(P)`** — извлекает ординату (координату $Y$) точки $P$.
> 
> **Пример использования:**
> ```json
> {
>   "type": "MathExpression",
>   "expression": "x(A) * y(B) - x(B) * y(A)",
>   "variables": {
>     "A": "A",
>     "B": "B"
>   }
...
* `{"type": "IsAcute", "args": {"points": ["A", "B", "C"]}}` — гарантирует, что угол ABC острый.
* `{"type": "Convex", "args": {"points": ["A", "B", "C", "D"]}}` — гарантирует выпуклость многоугольника.
* `{"type": "NonCollinear", "args": {"points": ["A", "B", "C"]}}` — гарантирует, что три точки не лежат на одной прямой.
* `{"type": "Inequality", "operator": ">", "left": {"type": "Distance", "points": ["A", "B"]}, "right": {"type": "Number", "value": 5}}` — математические неравенства.
---
