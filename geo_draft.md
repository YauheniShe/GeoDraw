...
> 
> Если внутри `MathExpression` требуется получить сырые декартовы координаты точек (например, для проверки коллинеарности или вычисления площадей в `constraints`), точка передается в блок `variables` как обычная строка-ссылка. 
> 
